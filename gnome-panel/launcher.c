		  ...
		  Launcher           *launcher)
{
	...
	if (info == TARGET_URI_LIST) {
		uri_list = g_strconcat ("file:", launcher->ditem->location,
					"\r\n", NULL);
		...
		gtk_selection_data_set (selection_data,
					...
					launcher->ditem->location,
					strlen (launcher->ditem->location));
	}
}
...
create_launcher (const char *parameters, GnomeDesktopItem *ditem)
{
	...
	if(ditem == NULL) {
		if (parameters == NULL) {
			...
			ditem = gnome_desktop_entry_load_unconditional (parameters);
		} else {
	...
	if (icon && *icon) {
		...
		launcher->button = button_widget_new(ditem->icon,
						     -1,
						     LAUNCHER_TILE,
						     FALSE,
						     ORIENT_UP,
...
properties_apply (Launcher *launcher)
{
	...
	/* save (steal) location */
	location = launcher->ditem->location;
