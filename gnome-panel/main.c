...
static int
get_window_id(Window win, char *title, Launcher *lclass, guint32 *wid)
{
	Window root_return;
	...
	if(title) {
		...
		if(XGetClassHint(GDK_DISPLAY(),win,&class_hint)) {
			int i;
			for(i=0;i<lclass->dentry->wm_classes_length;i++) {
				if(strcmp(lclass->dentry->wm_classes[i],class_hint.res_name)==0 ||
				   strcmp(lclass->dentry->wm_classes[i],class_hint.res_class)==0) {
					if(wid) *wid = win;
					ret = TRUE;
					break;
				}
	...
	for(i=0;!ret && i<nchildren;i++)
		ret=get_window_id(children[i],title,lclass,wid);
	if(children)
		XFree(children);
	return ret;
}
...
event_filter(GdkXEvent *gdk_xevent, GdkEvent *event, gpointer data)
{
	...
	if (xevent->type == MapNotify && (check_launchers || check_swallows)) {
		...
		remove = 0;
		for(li = check_launchers; li; li = g_list_next(li)) {
			Launcher *launcher = li->data;
			if(get_window_id(xevent->xmap.window,NULL,
					 launcher,
					 NULL)) {
		...
		while(remove--)
			check_launchers = g_list_remove(check_launchers,NULL);
